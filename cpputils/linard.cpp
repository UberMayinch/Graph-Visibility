...
int main(int argc, char** argv){
    ...
    G_TIME.clear(); G_X.clear(); G_Y.clear();
    ...
    for (uint32_t i = 0; i < rows; ++i) {
        ...
        const double xval = G_X[i];
        const double yval = G_Y[i];
        outfile.write(reinterpret_cast<const char*>(&tval), sizeof(tval));
        outfile.write(reinterpret_cast<const char*>(&xval), sizeof(xval));
        outfile.write(reinterpret_cast<const char*>(&yval), sizeof(yval));
    }
    outfile.close();
